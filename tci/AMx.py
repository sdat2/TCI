...
from tci.absolute_vorticity import absolute_vorticity
...
def do_tci(ifile, odir=None):
    ...
    if os.path.exists(ofile):
        ...
        chi.to_dataset(name=dname).to_netcdf(
            ofile,
    ...
    if os.path.exists(ofile):
        ...
        truncate_and_reverse_plevels = lambda x: x.sel(pfull=slice(60, None)).isel(
            pfull=slice(-1, None, -1)
        )  #
    ...
    if os.path.exists(ofile):
        ...
        Vshear.to_dataset(name=dname).to_netcdf(
            ofile,
            encoding={dname: {"dtype": "float32", "zlib": True, "complevel": 1}},
            unlimited_dims="time",
        )
    ...
    if os.path.exists(ofile):
        ...
        VI.to_dataset(name=dname).to_netcdf(
            ofile,
            encoding={dname: {"dtype": "float32", "zlib": True, "complevel": 1}},
            unlimited_dims="time",
        )
    ...
    if os.path.exists(ofile):
        ...
        eta = absolute_vorticity(vort850=ds.vort850, lat=ds.grid_yt)
        eta.to_dataset(name=dname).to_netcdf(
            ofile,
            encoding={dname: {"dtype": "float32", "zlib": True, "complevel": 1}},
            unlimited_dims="time",
        )
    ...
    if os.path.exists(ofile):
        ...
        H.attrs["units"] = "%"
        H.to_dataset(name=dname).to_netcdf(
            ofile,
            encoding={dname: {"dtype": "float32", "zlib": True, "complevel": 1}},
            unlimited_dims="time",
        )
    ...
    if os.path.exists(ofile):
        ...
        GPI.attrs[
            "history"
        ] = "|10**5\eta|**(3/2) * (H/50)**3 * (Vpot/70)**3 * (1+0.1*Vshear)**(-2)"
        GPI.to_dataset(name=dname).to_netcdf(
            ofile,
            encoding={dname: {"dtype": "float32", "zlib": True, "complevel": 1}},
            unlimited_dims="time",
        )
    ...
    if os.path.exists(ofile):
        ...
        GPI2010.attrs[
            "history"
        ] = "|\eta|**3 * chi**(-4/3) * max((Vpot-35),0)**2 * (25+Vshear)**(-4)"
        GPI2010.to_dataset(name=dname).to_netcdf(
            ofile,
            encoding={dname: {"dtype": "float32", "zlib": True, "complevel": 1}},
            unlimited_dims="time",
        )
